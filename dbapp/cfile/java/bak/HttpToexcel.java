...
public class HttpToexcel{
	public static <LabeledCSVParser> void main(String args[]){
		...
		try {
			out = new FileOutputStream("/datacenter/msa/report/msaexport/httprecord/httpexport.xls");
		...
		for (rownum = (short) 0; rownum < flag; rownum++){
			    ...
			    for (short cellnum = (short) 0; cellnum < 9; cellnum++){  
			    	     c = r.createCell(cellnum);       
			              try {     
						       ...
						       String csvFilePath = "/datacenter/msa/report/msaexport/httprecord/httpexport.csv";  
	     ...
	     for (int i = 0; i < rows; i++) {
	         ...
	         for (m = sourceRow.getFirstCellNum(); m < sourceRow.getLastCellNum(); m++) {
		         ...
		         if(i==1){//第二行
		        	 if(m==0){
		        		   ...
		        		   CellRangeAddress Hebin=new CellRangeAddress(1,1,1,8);
				 	       s.addMergedRegion(Hebin);
				 	       targetCell.setCellValue(new HSSFRichTextString("HTTP汇总记录查询"));	   
		         ...
		         if(i==6){//第八行
		        	 if(m==0){
		        		 targetCell.setCellValue("HTTP服务类型");		
		        		 targetCell.setCellStyle(normalStyle);
		        	 }else{
		         ...
		         if(i==9){
			        	...
			        	if(m==4){
			        		targetCell.setCellValue("域名");
			        	}
			        	if(m==5){
			        		targetCell.setCellValue("流量");
			        	}
			        	
			        	if(m==6){
			        		targetCell.setCellValue("访问次数");
			        	}
			        	if(m==7){
			        		targetCell.setCellValue("连接时间");
			        	}
			        	
			        	if(m==8){
			        		targetCell.setCellValue("HTTP服务分类");
			        	}
			        	targetCell.setCellStyle(normalStyle); 
