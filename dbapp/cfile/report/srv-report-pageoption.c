...
int toautoreportlist(int confd,ltMsgHead *ltMsgPk,lt_shmHead *lt_MMHead)
{
   ...
   if(ltMsgGetVar_s(ltMsgPk,"myfile")){
   	   filedir=ltMsgGetVar_s(ltMsgPk,"myfile");
   }
   if(ltMsgGetVar_s(ltMsgPk,"deldir")){
	 	   deldir=ltMsgGetVar_s(ltMsgPk,"deldir");
	 }
   if(filedir!=NULL){
	    sprintf(caCmd,"rm -Rf /datacenter/msa/report/%s/%s",deldir,filedir);
      system(caCmd);
      sleep(1);
   }
