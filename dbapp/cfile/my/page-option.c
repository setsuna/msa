...
int reporttempledit(int confd,ltMsgHead *ltMsgPk,lt_shmHead *lt_MMHead)
{
  ...
  char sqlBuf[1024], strBuf[1024];
	...
	temptype1=atol(temptypes1);
		...
		printf("topusernum::%s\n",topusernum);
		if(gid!=-1){
			sprintf(strBuf,"select level,name from msagroup where id=%ld",gid);
			tempCursor=ltDbOpenCursor(G_DbCon,strBuf);
			if(tempCursor!=NULL){
				tempRow= ltDbFetchRow(tempCursor);
				if(tempRow!=NULL){
					glevel=atol(tempRow[0]);
					sprintf(group,"%s",tempRow[1]);
				}
			}
			...
			printf("group::%s\n",group);  
			if(userid!=-1){
				sprintf(sqlBuf,"select dispname from msauser where userid=%ld and groupid%d=%ld",userid,glevel,gid);
				printf("sqlBuf::%s\n",sqlBuf);  
				tempCursor=ltDbOpenCursor(G_DbCon,sqlBuf);
				if(tempCursor!=NULL){
					tempRow= ltDbFetchRow(tempCursor);
					if(tempRow!=NULL){
						sprintf(dispname,"%s",tempRow[0]);
					}
				}
				ltDbCloseCursor(tempCursor);    
				lt_dbput_rootvars(dbPtr,1,"user",dispname);
			}else{
				char select[64];
				char tmpselect[64];
				memset(select,0,sizeof(select));
				memset(tmpselect,0,sizeof(tmpselect));
				sprintf(select,"%s","请选择");
				strgb2utf8(select,tmpselect,strlen(select));
				memset(select,0,sizeof(select));
				sprintf(select,"%s",tmpselect);
				lt_dbput_rootvars(dbPtr,1,"user",select);
			}
		}
	  ...
	  lt_dbput_rootvars(dbPtr,2,"xgid",gids,"xuserid",userids);
	 
	  lt_dbput_html(confd,ltMsgPk,dbPtr->doc,"/app/msa/msa/htmlplt/reportcenter/reportstylesettingedit.htm",dbPtr->head,0);
	  ltMsgFree(ltMsgPk); 
...
{ 
   ...
   lTime = time(0);
	 ...
	 printf("topusernum:%s\n",topusernum);
	 printf("createDate::%s\n",createDate);
	 printf("deployscope0::%d\n",deployscope0);
	 printf("deployscope1::%d\n",deployscope1);
...
int templatereportlista(int confd,ltMsgHead *ltMsgPk,lt_shmHead *lt_MMHead)
{
   ...
   lt_dbput_html(confd,ltMsgPk,dbPtr->doc,"/app/msa/msa/htmlplt/reportcenter/templatereportlist.htm",dbPtr->head,0);  
   ltMsgFree(ltMsgPk);
   lt_dbfree(dbPtr);
...
int reportcreate(int confd,ltMsgHead *ltMsgPk,lt_shmHead *lt_MMHead)
{
  ...
  char **binqqargs;
 /*************************************
  *0 1 2 3    	binqqargs[0]表示第一行*
  *1 a b c      binqqargs[1]表示第二行*
  *************************************/
  int  iPid;//子进程标记
	ltDbHeadPtr dbPtr;
	long lTime;
  ...
  lt_db_htmlpage(dbPtr,"utf-8");
	lTime=time(0);//unix时间表示为距离1970:1月1号:00:00:00起距现在的秒数
