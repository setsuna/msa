...
{	 
   //系统状态分析报告	
    ...
    nasTimeGetDate(caSdate2,time2);
		...
		sprintf(sqlBuf,"select sdate,cast(avg(ubytes) as dec(18,2)) as ubytes,cast(avg(dbytes) as dec(18,2)) as dbytes,cast(avg(tcpconnect) as dec(18,2)) as tcpconnect,cast(avg(udpconnect) as dec(18,2)) as udpconnect,cast(avg(onlineuser) as dec(18,2)) as onlineuser\
		 from msatimereport where substring(sdate,3,8)>='%s' and substring(sdate,3,8)<='%s' group by sdate order by sdate asc",caSdate1,caSdate2);
...
int sysstep3(int confd,ltMsgHead *ltMsgPk,lt_shmHead *lt_MMHead)
{
  ...
  sprintf(caTempDataDir,"%s/data3",caTempDir);
	...
	sprintf(sqlBuf,"%s","select userid,sdate,ubytes+dbytes as flow,ctime,onlinetime from msasrvreport order by flow desc");
	printf("SQL:  %s \n",sqlBuf);
  tempCursor=ltDbOpenCursor(G_DbCon,sqlBuf);
  if(tempCursor!=NULL){
	    tempRow= ltDbFetchRow(tempCursor);
  ...
  num=0;
  memset(sqlBuf,0,sizeof(sqlBuf));
  sprintf(sqlBuf,"select cast(avg(msasrvreport.ubytes+msasrvreport.dbytes) as dec(18,3)) as flow,cast(avg(msasrvreport.onlinetime) as dec(18,3)) as onlinetime,msauser.groupid0 from msasrvreport left join msauser on msasrvreport.userid=msauser.userid where msasrvreport.sdate<'%s' and msasrvreport.sdate>'%s' group by msauser.groupid0 order by flow desc",caEdate,caSdate);
	printf("gourp SQL:  %s \n",sqlBuf);
  tempCursor=ltDbOpenCursor(G_DbCon,sqlBuf);
  if(tempCursor!=NULL){
	    tempRow= ltDbFetchRow(tempCursor);
...
int sysstep4(int confd,ltMsgHead *ltMsgPk,lt_shmHead *lt_MMHead)
{
  ...
  if(tempCursor!=NULL){
    	...
    	while(tempRow!=NULL){
           ...
           sprintf(sortname,"%s",_ltPubInfo->_urlSortlist[atoi(tempRow[5])].sortname);
           sprintf(numchar,"%d",num);
    ...
    if(tempCursor!=NULL){
	    	...
	    	while(tempRow!=NULL){
               ...
               sprintf(flow,"%0.3fKB",atoll(tempRow[0])/1024.00);
           	 ...
           	 sprintf(sortname,"%s",_ltPubInfo->_urlSortlist[atoi(tempRow[3])].sortname);
             sprintf(numchar,"%d",num);
	    ...
	    sprintf(uname,"username%d",i);
		  ...
		  if(tempCursor!=NULL){
		    	...
		    	while(tempRow!=NULL){
							 ...
							 //URL分类
							 memset(sortname,0,sizeof(sortname));
							 sprintf(sortname,"%s",_ltPubInfo->_urlSortlist[atoi(tempRow[1])].sortname);
